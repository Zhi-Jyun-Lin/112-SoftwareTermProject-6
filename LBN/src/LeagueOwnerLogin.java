...
public class LeagueOwnerLogin implements ActionListener {
	// 查詢帳號，進行認證
	public String check(String username, String userpassword) {
		...
		try {
			...
			String sql;
			sql = "SELECT  leagueownerId, password FROM leagueownerAccount";
			ResultSet rs = stmt.executeQuery(sql);

			while (rs.next()) {
				String leagueownerId = rs.getString("leagueownerId");
				String password = rs.getString("password");

				if ((leagueownerId.equals(username) && password.equals(userpassword))) {
					answer = "true";
					break;
				}
			}
	...
	LeagueOwnerLogin() {
		...
		forgetLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				frame.dispose();
				LeagueForgetPassword tf = new LeagueForgetPassword();
			}
		});
		...
		registerLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					frame.dispose();
					LeagueOwnerAssign assign = new LeagueOwnerAssign();
		...
		JLabel title = new JLabel("你好，聯盟者");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == login) {
			...
			if (type.equals("true")) {
				frame.dispose();
				LeagueOwnerPage sys = new LeagueOwnerPage();
			} else {
	 ...
	 * usertext.getText(); String userpassword = passwordtext.getText();
	 * System.out.println(username); System.out.println(userpassword); AdminLogin
	 * login = new AdminLogin(); String type = login.check(username, userpassword);
	 * 
	 * if (type.equals("true")) { frame.dispose(); AdminPage sys = new AdminPage();
	 * } else if (e.getSource() == turnback) { frame.dispose(); CharacterGui cha =
	 * new CharacterGui(); } else if (e.getSource() == forgetLabel) { // Corrected
	 * variable name frame.dispose(); AdminForgetPassword tf = new
	 * AdminForgetPassword(); }else if (type.equals("true")) { frame.dispose();
