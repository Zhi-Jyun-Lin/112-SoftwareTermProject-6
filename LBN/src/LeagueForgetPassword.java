...
public class LeagueForgetPassword implements ActionListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					LeagueForgetPassword window = new LeagueForgetPassword();
	...
	// 更改密碼
		public void alterpass(String leagueownerId, String password) {
			Connection c = null;
			Statement stmt = null;
			try {
				...
				String sql;
				sql = "UPDATE leagueownerAccount SET password='" + password + "'" + " WHERE leagueownerId='" + leagueownerId + "'";
		...
		public String check(String userid) {
			...
			try {
				...
				stmt = c.createStatement();
				String sql;
				sql = "SELECT leagueownerId FROM leagueownerAccount";
				ResultSet rs = stmt.executeQuery(sql);
				...
				while (rs.next()) {
					...
					if (leagueownerId.equals(userid)) {
						answer = "true";
						break;
					}
	    ...
	    public void actionPerformed(ActionEvent e) {
	        ...
	        if (e.getSource() == ensure) {// 確認修改密碼
	            if (user.isEmpty() || password.isEmpty()) {
	                ...
	                result.setVisible(true);
	                userframe.dispose();
	                LeagueOwnerLogin sl = new LeagueOwnerLogin();
	            }
