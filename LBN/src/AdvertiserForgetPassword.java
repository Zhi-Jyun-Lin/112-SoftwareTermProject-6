...
public class AdvertiserForgetPassword implements ActionListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AdvertiserForgetPassword window = new AdvertiserForgetPassword();
	 ...
	 */
	public AdvertiserForgetPassword() {
		initialize();
	}
	...
	// 更改密碼
		public void alterpass(String advertiserId, String password) {
			Connection c = null;
			Statement stmt = null;
			try {
				...
				String sql;
				sql = "UPDATE advertiserAccount SET password='" + password + "'" + " WHERE advertiserId='" + advertiserId + "'";
		...
		public String check(String userid) {
			...
			try {
				...
				stmt = c.createStatement();
				String sql;
				sql = "SELECT advertiserId FROM advertiserAccount";
				ResultSet rs = stmt.executeQuery(sql);
	    ...
	    public void actionPerformed(ActionEvent e) {
	        ...
	        if (e.getSource() == ensure) {// 確認修改密碼
	            if (user.isEmpty() || password.isEmpty()) {
	                ...
	                result.setVisible(true);
	                userframe.dispose();
	                AdvertiserLogin sl = new AdvertiserLogin();
	            }
