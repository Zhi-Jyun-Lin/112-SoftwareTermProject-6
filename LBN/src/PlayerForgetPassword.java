...
public class PlayerForgetPassword implements ActionListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					PlayerForgetPassword window = new PlayerForgetPassword();
	...
	// 更改密碼
		public void alterpass(String playerId, String password) {
			Connection c = null;
			Statement stmt = null;
			try {
				...
				String sql;
				sql = "UPDATE playerAccount SET password='" + password + "'" + " WHERE playerId='" + playerId + "'";
		...
		public String check(String userid) {
			...
			try {
				...
				stmt = c.createStatement();
				String sql;
				sql = "SELECT playerId FROM playerAccount";
				ResultSet rs = stmt.executeQuery(sql);
	    ...
	    public void actionPerformed(ActionEvent e) {
	        ...
	        if (e.getSource() == ensure) {// 確認修改密碼
	            if (user.isEmpty() || password.isEmpty()) {
	                ...
	                result.setVisible(true);
	                userframe.dispose();
	                PlayerLogin sl = new PlayerLogin();
	            }
